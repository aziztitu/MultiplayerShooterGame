...
public class GameManagerOld
{
    ...
    {
        get { return _instance ?? (_instance = new GameManagerOld()); }
    }
    ...
    private class GameDataPreview
    {
        ...
        {
            if (!GameManagerOld.Instance._previewGameData)
                return;

            _gameDataPreviewItems.Clear();

            foreach (KeyValuePair<GameDataKey, string> keyValuePair in GameManagerOld.Instance._gameData)
            {
